...
public class EncryptUtil {
    ...
    public static void generateKeyPair(Long startTime, String licenseCode, String pubPath, String priPath) {
        try {
            PublicAndPrivateKey publicAndPrivateKey = generateKeyPair(startTime, licenseCode);
            // 保存文件
            if (pubPath != null) {
                FileUtils.writeStringToFile(new File(pubPath), publicAndPrivateKey.getPubKey(), String.valueOf(StandardCharsets.UTF_8));
            }
            if (priPath != null) {
                FileUtils.writeStringToFile(new File(priPath), publicAndPrivateKey.getPriKey(), String.valueOf(StandardCharsets.UTF_8));
            }
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @return
     */
    public static PublicAndPrivateKey generateKeyPair(Long startTime, String licenseCode) {
        PublicAndPrivateKey publicAndPrivateKey = new PublicAndPrivateKey();
        try {
            //  创建密钥对生成器对象
            ...
            publicAndPrivateKey.setPubKey(publicKeyString);
            publicAndPrivateKey.setPriKey(privateKeyString);
        } catch (Exception e) {
    ...
    public static PrivateKey loadPrivateKeyFromString(String keyString) {
        try {
            ...
            throw new RuntimeException("获取私钥失败！"+e.getMessage());
        }
    }
